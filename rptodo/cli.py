...
def list_all() -> None:
    ...
    typer.secho(headers, fg=typer.colors.WHITE, bold=True)
    typer.secho("-" * len(headers), fg=typer.colors.WHITE)
...
def set_done(todo_id: int = typer.Argument(...)) -> None:
    """Complete a to-do by settingit as done using its TODO_ID"""
    todoer = get_todoer()
...
def remove_all(
    force: bool = typer.Option(
        ...
        help="Force deletionm without confirmation.",
    ),
) -> None:
    """Remove all to-dos."""
    todoer= get_todoer()
...
def get_todoer() -> rptodo.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        ...
        typer.secho(
            'Config file note found. Please, run "rptodo init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    if db_path.exists():
...
def main(
    version: Optional[bool] = typer.Option(
        None, "--version", "-v", 
        help="Show's the application's version and exit.",
